...
fun main() {
    ...
    when (readLine()!!) {
        ...
        "3" -> cityNameSort(nameArray, addressArray, cityArray)
        "4" -> zipCodeSort(nameArray, addressArray, cityArray)
        "quit" -> exitProcess(0)
    }
    main()
}
...
fun cityNameSort(nameArray: Array<String>, addressArray: Array<String>, cityArray: Array<String>) {
    // Temporary buffer for storing the value of the position after the one being compared
    var bubbleBuffer: String
    // While loop that iterates until the array is sorted
    while (true) {
...
fun zipCodeSort(nameArray: Array<String>, addressArray: Array<String>, cityArray: Array<String>) {
    // Temporary buffer for storing the value of the position after the one being compared
    var bubbleBuffer: String
    // While loop that iterates until the array is sorted
    while (true) {
        ...
        for (i in 0 until nameArray.size) {
            try {
                // If the pair is not in alphabetical order, it swaps them
                if (cityArray[i].substring(cityArray[i].indexOf(", ") + 4) > cityArray[i + 1].substring(cityArray[i + 1].indexOf(", ") + 4)) {
                    // Swaps around the nameArray
                    bubbleBuffer = nameArray[i + 1]
                    nameArray[i + 1] = nameArray[i]
                    nameArray[i] = bubbleBuffer
